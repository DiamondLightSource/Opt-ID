...
- an existing directory in which all the output data will be written to
...
This can be passed multiple `.genome` and `.inp` files to create a genome report
with some useful data visualisation as a PDF file.

Opt-ID will look in both the `genomes/` and `process_genome_output/` directories
so the filenames passed to `generate_report.sh` should only be the filenames and
not filepaths; for example, the correct way to pass a genome and a `.inp` file
to the script would be

```
./generate_report.sh foo.genome bar.inp
```

as opposed to

```
./generate_report.sh genomes/foo.genome process_genome_output/bar.inp
```
...
```
./generate_report --report-filename report.pdf foo.genome bar.inp
```

### `restart_sort.sh`

This script requires no parameters and can be run simply as `./restart_sort.sh`,
Opt-ID will take care of loading the YAML config of the previous sort job and
will use all the same flags and paramters as the original sort job. One example
is that if the original sort job was run on a cluster, so will the restart-sort
job, and another example is that the same `.json`, `.mag` and `.h5` (lookup
table) files from the original sort job will be reused in the restart-sort job
...
```
./compare_shim.sh --diff-filename my_shim foo.genome
```
